...
class FaissVectorDBStorage(BaseVectorStorage):
    """
    A Faiss-based Vector DB Storage for MultimodalRAG.
    ...
    def __post_init__(self):
        ...
        if self.global_config["unified_vector_storage"]:
            storage_dir = self.global_config["vector_storage_dir"]
        else:
            storage_dir = self.global_config["working_dir"]

        self._faiss_index_file = os.path.join(
            storage_dir, f"faiss_index_{self.namespace}.index"
        )
        self._meta_file = self._faiss_index_file + ".meta.json"

        if self.global_config.get("remove_existing", False):
            if os.path.exists(self._faiss_index_file):
                logger.info(f"Removing existing Faiss index file: {self._faiss_index_file}")
                os.remove(self._faiss_index_file)
            if os.path.exists(self._meta_file):
                logger.info(f"Removing existing metadata file: {self._meta_file}")
                os.remove(self._meta_file)

        self._max_batch_size = self.global_config["embedding_batch_num"]
        # Embedding dimension (e.g. 768) must match your embedding function
        
        self._dim = self.embedding_func.embedding_dim if self.embedding_func else 768
    ...
    async def upsert(self, data: dict[str, dict[str, Any]]) -> None:
        ...
        for i, meta in enumerate(list_data):
            self._id_to_meta[start_idx + i] = meta

        logger.info(f"Upserted {len(list_data)} vectors into Faiss index.")
        return [m["__id__"] for m in list_data]
    
    async def merge(self, vector_db: BaseVectorStorage) -> None:
        """
        Merge vectors from another vector DB storage into this one.
        """
        logger.info(f"Merging vectors from {vector_db.namespace} into {self.namespace}")

        if not vector_db._id_to_meta:
            return
    
        list_data = []
        embeddings = []
        for fid, meta in vector_db._id_to_meta.items():
            list_data.append(meta)
            embeddings.append(vector_db._index.reconstruct(fid))
        ...
        logger.info(f"Merged {len(list_data)} vectors into Faiss index.")
        logger.info(f"Total vectors: {self._index.ntotal}")

    async def query(self, query: str | np.ndarray, top_k: int) -> list[dict[str, Any]]:
        """
        Search by a textual query or image query; returns top_k results with their metadata + similarity distance.
        """
        ...
        logger.info(
            f"Query: top_k: {top_k}, threshold: {self.cosine_better_than_threshold}"
        )
    ...
    async def get_embedding_by_custom_id(self, custom_ids: list[str]) -> list[np.ndarray]:
        """
        Return the embedding vector for a given custom ID.
        """
        embeddings = []
        for cid in custom_ids:
            fid = self._find_faiss_id_by_custom_id(cid)
            if fid is not None:
                embeddings.append(self._index.reconstruct(fid))
            else:
                raise ValueError(f"Custom ID {cid} not found in {self.namespace}")
        embeddings = np.array(embeddings, dtype=np.float32)
        return embeddings

    @property
    def client_storage(self):
        # Return whatever structure mKG-RAG might need for debugging
    ...
    async def delete_entity(self, entity_name: str) -> None:
        """
        Delete vectors for a given entity by scanning metadata.
        """
        logger.debug(f"Attempting to delete entity {entity_name}")
        for fid, meta in self._id_to_meta.items():
            if meta.get("__id__") == entity_name:
                self._remove_faiss_ids([fid])
                logger.debug(f"Deleted entity {entity_name}")
                return
        logger.debug(f"Entity {entity_name} not found in {self.namespace}")
    ...
    def _remove_faiss_ids(self, fid_list):
        ...
        for new_fid, old_fid in enumerate(keep_fids):
            vectors_to_keep.append(self._index.reconstruct(old_fid))  # stored as list
            new_id_to_meta[new_fid] = self._id_to_meta[old_fid]
    ...
    def _save_faiss_index(self):
        ...
        # _id_to_meta is { int: { '__id__': doc_id, ... } }
    ...
    def _load_faiss_index(self):
        ...
        if not os.path.exists(self._faiss_index_file):
            logger.info("No existing Faiss index file found. Starting fresh.")
            return
